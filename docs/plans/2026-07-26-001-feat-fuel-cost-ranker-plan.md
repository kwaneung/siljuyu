...
- **차량 프리셋 + 공인연비.** 온보딩 프리셋은 `그랜저 3.0 가솔린`처럼 차종+유종을 한 세트로 제공하고, 각 프리셋에 해당 차량의 **공인연비(km/L)** 를 포함한다. 선택 시 공인연비가 계산용 연비 초기값으로 채워지며, 사용자는 실연비로 수정할 수 있다. 목록에 없는 차는 직접 입력한다. `(session-settled: user-directed — chosen over fuel-type-only presets; 공인연비 포함은 user-directed)`
- **기기 저장, 무로그인.** 연비·유종·주유량·반경·온보딩 완료 여부는 브라우저에 기억한다. `(session-settled: user-approved — chosen over accounts: v1 마찰 최소화)`
- **길안내는 외부 지도.** 상세에서 카카오/네이버/구글 등 외부 지도 앱·웹으로 딥링크한다. 앱 내 지도는 v1에 넣지 않는다. `(session-settled: user-approved)`
...
- F1. 첫 방문 온보딩
  ...
  - **Actors:** A1
  - **Steps:** 위치 권한을 요청한다. 차량+유종 프리셋(공인연비 표시)을 고르거나 직접 입력한다. 공인연비로 채워진 연비를 확인하고 필요 시 실연비로 수정한다. 주유량(프리셋 또는 리터 입력)과 반경(3/5/10/20km)을 확정한다. 네 값이 모두 확정되기 전에는 랭킹으로 진행할 수 없다. 완료 시 기기에 저장하고 랭킹으로 이동한다.
...
- R2. 온보딩은 `차종 + 유종`이 결합된 차량 프리셋을 제공하고, 각 프리셋에 해당 차량의 공인연비(km/L)를 포함한다. 선택 시 공인연비가 연비 초기값으로 채워지며, 사용자는 이를 수정할 수 있어야 한다. 프리셋에 없는 차는 직접 입력할 수 있어야 한다.
...
- AE2. 프리셋 공인연비 적용 후 보정
  - **Covers:** R2
  - **Given:** 온보딩에서 `그랜저 3.0 가솔린` 프리셋(공인연비 예: 10.8km/L)을 고른다
...
- Q5. 오피넷 API 키·프록시·호출 한도를 제품/배포 환경에서 어떻게 둘 것인가.
