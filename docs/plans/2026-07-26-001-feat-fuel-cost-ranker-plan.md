...
- **차량 프리셋.** 온보딩 프리셋은 `그랜저 3.0 가솔린`처럼 차종+유종을 한 세트로 제공한다. 적용 후 연비 숫자는 수정 가능하다. 목록에 없는 차는 직접 입력한다. `(session-settled: user-directed — chosen over fuel-type-only presets)`
...
- F1. 첫 방문 온보딩
  ...
  - **Actors:** A1
  - **Steps:** 위치 권한을 요청한다. 차량+유종 프리셋을 고르거나 직접 입력한다. 연비를 확인하고 필요 시 수정한다. 주유량(프리셋 또는 리터 입력)과 반경(3/5/10/20km)을 확정한다. 네 값이 모두 확정되기 전에는 랭킹으로 진행할 수 없다. 완료 시 기기에 저장하고 랭킹으로 이동한다.
...
- R2. 온보딩은 `차종 + 유종`이 결합된 차량 프리셋을 제공하고, 적용 후 연비는 수정할 수 있어야 한다. 프리셋에 없는 차는 직접 입력할 수 있어야 한다.
...
- AE2. 프리셋 후 연비 보정
  - **Covers:** R2
  - **Given:** 온보딩에서 `그랜저 3.0 가솔린` 프리셋을 고른다
  - **When:** 채워진 연비를 11.2로 고치고 저장한다
  - **Then:** 이후 총비용 계산은 11.2km/L과 해당 유종을 사용한다
...
- 차량 프리셋 연비는 대표값이며, 실연비는 사용자가 보정한다.
...
- Q2. 차량 프리셋 목록의 규모와 구체 차종·대표 연비 출처.
