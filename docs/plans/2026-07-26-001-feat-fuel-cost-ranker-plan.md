...
위치는 브라우저 권한을 우선하고, 거절 시에는 주소/핀 수동 입력으로 기준 위치를 받는다.
...
- **위치 거절 시 수동 입력.** 위치 권한이 없거나 거절되면 주소 또는 핀으로 기준 위치를 수동 입력해 랭킹을 계속한다. 권한 재요청 안내만으로 막지 않는다. `(session-settled: user-directed — chosen over permission-only gate)`
...
- F1. 첫 방문 온보딩
  ...
  - **Actors:** A1
  - **Steps:** 위치 권한을 요청한다. 허용되면 GPS 좌표를 쓰고, 거절·실패 시 주소/핀 수동 입력으로 기준 위치를 받는다. 차종+유종+공인연비 프리셋을 고르거나 직접 입력한다. 공인연비로 채워진 연비를 확인하고 필요 시 실연비로 수정한다. 주유량(프리셋 또는 리터 입력)과 반경(3/5/10/20km)을 확정한다. 기준 위치와 네 값이 모두 확정되기 전에는 랭킹으로 진행할 수 없다. 완료 시 기기에 저장하고 랭킹으로 이동한다.
...
- R12. 위치 권한이 거절되거나 위치를 얻을 수 없으면, 주소 또는 핀 수동 입력으로 기준 위치를 받아 랭킹을 계속할 수 있어야 한다.
...
- AE6. 위치 거절 후 수동 입력
  ...
  - **When:** 주소 또는 핀으로 기준 위치를 입력하고 온보딩/설정을 마친다
...
- 길안내는 외부 지도 딥링크만 사용한다(앱 내 지도 없음). 기준 위치 수동 입력용 UI는 길안내용 앱 내 지도와 별개이며, 임베드 지도 SDK 없이도 주소 검색·좌표 입력으로 충족할 수 있다.
...
- Q1. 수동 위치 입력 UX를 주소 검색 중심으로 할지, 좌표/간단 핀 선택까지 둘지.
